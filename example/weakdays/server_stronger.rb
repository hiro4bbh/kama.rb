...
class WEBrick::HTTPUtils::FormData
  alias :old_to_s :to_s
  def to_s
    # FIXME: This is an ad-hoc solution for the placeholders in SQLite3::Database#execute.
    # I don't know why not any object of this class but the only objects passed by WEBrick causes this problem...
    # Maybe, WEBrick uses some tricks for efficiently retaining string values...
    "#{self.old_to_s}"
  end
end
...
class Database < SQLite3::Database
  ...
  def create_calendar(calname, uid, color: 'blue')
    execute('INSERT INTO calendars(calname, uid, color) VALUES (?, ?, ?);', bind_vars=[calname.to_s, uid&.to_i, color.to_s])  # FIXED(SQLI 1, 2)
  end
  def create_event(cid, uid, start_at, finish_at, title, content)
    ...
    execute('INSERT INTO events(cid, uid, start_at, finish_at, title, content) VALUES (?, ?, ?, ?, ?, ?);', bind_vars=[cid&.to_i, uid&.to_i, start_at&.to_i, finish_at&.to_i, title.to_s, content.to_s])  # FIXED(SQLI 6, 7, 9)
  end
  def create_user(username, password)
    execute('INSERT INTO users(username, password) VALUES (?, ?)', bind_vars=[username.to_s, password.to_s])  # FIXED(SQLI 17)
  end
  ...
  def edit_calendar(cid, calname, color)
    execute('UPDATE calendars SET calname = ?, color = ? WHERE cid = ?;', bind_vars=[calname.to_s, color.to_s, cid&.to_i])  # FIXED(SQLI 3, 5)
  end
  def edit_event(eid, cid, start_at, finish_at, title, content)
    start_at, finish_at = start_at.to_time.to_i, finish_at.to_time.to_i
    raise "Finish time should be later than start time" unless start_at < finish_at
    execute('UPDATE events SET cid = ?, start_at = ?, finish_at = ?, title = ?, content = ? WHERE eid = ?;', bind_vars=[cid&.to_i, start_at&.to_i, finish_at&.to_i, title.to_s, content.to_s, eid&.to_i])  # FIXED(SQLI 10, 11, 14)
  end

  def select_calendar(cid)
    res = execute("SELECT * FROM calendars WHERE cid = ? LIMIT 0, 1;", bind_vars=[cid&.to_i])  # FIXED(SQLI 4)
  ...
  def select_event(eid)
    res = execute("SELECT * FROM events INNER JOIN calendars ON events.cid = calendars.cid WHERE eid = ? LIMIT 0, 1;", bind_vars=[eid&.to_i])  # FIXED(SQLI 12)
    return nil if res.length == 0
  ...
  def select_user(username)
    res = execute("SELECT * FROM users WHERE username = ? LIMIT 0, 1;", bind_vars=[username&.to_s])  # FIXED(SQLI 15, 16)
    return nil if res.length == 0
...
class Server
  ...
  def generate_form_input(res, label, name, value: '',  type: 'text', style: '')
    res.body << "<tr><th><label>#{label}</label></th><td><input type=\"#{type}\" name=\"#{name}\" value=\"#{CGI.escape_html(value.to_s)}\" style=\"#{style}\"></td></tr>"  # FIXED(XSS 1.B, 2.B, 4.B, 6.B)
  end
  ...
  def generate_form_select_option(res, label, value, clazz: nil, selected: false)
    res.body << "<option #{if clazz then "class=\"#{clazz}\"" else '' end} value=\"#{CGI.escape_html(value.to_s)}\" #{if selected then 'selected' else '' end}>#{CGI.escape_html(label.to_s)}</option>"  # FIXED(XSS 1.D, 1.E, 2.D, 2.E)
  end
  ...
  def generate_form_textarea(res, label, name, value: '', style: '')
    res.body << "<tr><th><label>#{label}</label></th><td><textarea name=\"#{name}\" style=\"#{style}\">#{CGI.escape_html(value.to_s)}</textarea></tr>"  # FIXED(XSS 3.A, 5.A)
  end
  ...
  def handler_calendar(req, res)
    ...
    when 'list'
      ...
      calendars.each do |cal|
        ...
        res.body << "&nbsp; <span class=\"title\">#{CGI.escape_html(cal[:calname].to_s)}</span>"  # FIXED(XSS 1.C, 2.C)
        res.body << '</div>'
      end
  ...
  def handler_index(req, res)
    ...
    result[:days].each do |day|
      ...
      day[:events].each do |event|
        ...
        res.body << "<span class=\"event-title\">#{CGI.escape_html(event[:title].to_s)}</span> <a class=\"event-link\" href=\"/event?action=edit&eid=#{event[:eid]}\">[EDIT]</a><br>"  # FIXED(XSS 4.A, 6.A)
        res.body << "<span class=\"event-time\">#{time_str}</span> <span class=\"event-calname\">(#{CGI.escape_html(event[:calname].to_s)})</span></div>"  # FIXED(XSS 1.A, 2.A)
      end
      res.body << '</td></tr>'
    end
